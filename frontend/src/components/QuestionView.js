...
class QuestionView extends Component {
    ...
    getQuestions = () => {
        $.ajax({
            url: proxy['proxy'] + `questions?page=${this.state.page}`,
            type: "GET",
            success: (result) => {
                this.setState({
    ...
    getByCategory = (id) => {
        $.ajax({
            url: proxy['proxy'] +`/categories/${id}/questions`,
            type: "GET",
            success: (result) => {
                this.setState({
                    questions: result.questions,
    ...
    submitSearch = (searchTerm) => {
        $.ajax({
            url: proxy['proxy'] +`/search/question`,
            type: "POST",
            dataType: 'json',
                    ...
                    <ul>
                        {Object.keys(this.state.categories).map((id,) => (
                            <li key={this.state.categories[id].id} onClick={() => {
                               ...
                               <p> {this.state.categories[id].type}
